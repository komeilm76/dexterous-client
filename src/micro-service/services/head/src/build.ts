...
const build = (
  config: ReturnType<typeof generator.makeHead>,
  entryOptions: IOption,
) => {
  // log
  const step = { value: 0 };
  const logLabel = "[micro/head]($)";
  ...
  jetpack.readAsync(fullNameInReadDir, "utf8").then(async (readData) => {
    tools.logger.log(step, logLabel, "readed file from 'readDir'");
    ...
    if (readData) {
      ...
      if (haveStartLabel && haveEndLabel) {
        ...
        tools.logger.log(step, logLabel, "data was pretty");
